...
// RATIOS
// -----------------------------------------------------------------------------------------------------------------------------------
// TODO: fix by doing switch case on which rate needs to picked for which characteristic (cringe brute force lowk)


const completeMissingAgeRatio = (root, characteristic, education, age) => {
    // TODO
    let completionObject = {
        calculable: false,
        calculation_queue: []
    ...
    let target = root[characteristic]["women"][education][age];
    if (target != "") {
        completionObject.calculable = true;
        completionObject.calculation_queue.push(target);
        return completionObject;
    ...
    let populationCompletion = completeMissingAgePopulation(root, "women", education, age);
    // target == "" ensures that calculable isnt false if you have the rate
    if (!populationCompletion.calculable && target == "") return completionObject;
    if (characteristic == "population") return populationCompletion;

    let age_15p_ratio = root[characteristic]["women"][education]["15+"];
    let age_15p_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "15+");
    let age_15p_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "15+");

    let age_15_24_ratio = root[characteristic]["women"][education]["15-24"];
    let age_15_24_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "15-24");
    let age_15_24_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "15-24");


    let age_25p_ratio = root[characteristic]["women"][education]["25+"];
    let age_25p_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "25+");
    let age_25p_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "25+");

    let age_25_54_ratio = root[characteristic]["women"][education]["25-54"];
    let age_25_54_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "25-54");
    let age_25_54_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "25-54");


    let age_55p_ratio = root[characteristic]["women"][education]["55+"];
    let age_55p_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "55+");
    let age_55p_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "55+");
    ...
    switch (age) {
        ...
        default:
            if (age_15p_ratio != "") {
                ...
                completionObject.calculation_queue.push(age_15p_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (age_15p_female_rate_CO.calculable && age_15p_male_rate_CO.calculable) {

                completionObject.calculation_queue.push(...age_15p_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...age_15p_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
        ...
        case "15-24":
            if (age_15_24_ratio != "") {
                ...
                completionObject.calculation_queue.push(age_15_24_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (age_15_24_female_rate_CO.calculable && age_15_24_male_rate_CO.calculable) {

                completionObject.calculation_queue.push(...age_15_24_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...age_15_24_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
        ...
        case "25+":
            if (age_25p_ratio != "") {
                ...
                completionObject.calculation_queue.push(age_25p_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (age_25p_female_rate_CO.calculable && age_25p_male_rate_CO.calculable) {

                completionObject.calculation_queue.push(...age_25p_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...age_25p_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
        ...
        case "25-54":
            if (age_25_54_ratio != "") {
                ...
                completionObject.calculation_queue.push(age_25_54_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (age_25_54_female_rate_CO.calculable && age_25_54_male_rate.CO_calculable) {

                completionObject.calculation_queue.push(...age_25_54_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...age_25_54_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
        ...
        case "55+":
            if (age_55p_ratio != "") {
                ...
                completionObject.calculation_queue.push(age_55p_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (age_55p_female_rate_CO.calculable && age_55p_male_rate_CO.calculable) {

                completionObject.calculation_queue.push(...age_55p_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...age_55p_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
...
export const completeMissingEducationRatio = (root, characteristic, education, age) => {
    ...
    let populationCompletion = completeMissingEducationPopulation(root, "women", education, age);
    // target == "" ensures that calculable isnt false if you have the rate
    if (!populationCompletion.calculable && target == "") return completionObject;
    if (characteristic == "population") return populationCompletion;

    let totalEducation_ratio = root[characteristic]["women"][education]["15+"];
    let totalEducation_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "15+");
    let totalEducation_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "15+");

    let underHighSchool_ratio = root[characteristic]["women"][education]["15-24"];
    let underHighSchool_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "15-24");
    let underHighSchool_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "15-24");

    let somePostSec_25p_ratio = root[characteristic]["women"][education]["25+"];
    let somePostSec_25p_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "25+");
    let somePostSec_25p_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "25+");

    let donePostSec_ratio = root[characteristic]["women"][education]["25-54"];
    let donePostSec_male_rate_CO =  completeMissingAgeRate(root, characteristic, "men", education, "25-54");
    let donePostSec_female_rate_CO =  completeMissingAgeRate(root, characteristic, "women", education, "25-54");
    ...
    switch (education) {
        ...
        case "total-education":
        default:
            if (totalEducation_ratio != "") {

                completionObject.calculation_queue.push(totalEducation_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (totalEducation_female_rate_CO.calculable && totalEducation_male_rate_CO.calculable) {
        ...
        case "less-than-high-school":
            if (underHighSchool_ratio != "") {

                completionObject.calculation_queue.push(underHighSchool_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (underHighSchool_female_rate_CO.calculable && underHighSchool_male_rate_CO.calculable) {

                completionObject.calculation_queue.push(...underHighSchool_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...underHighSchool_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
        ...
        case "high-school-or-some-postsecondary":
            if (somePostSec_25p_ratio != "") {

                completionObject.calculation_queue.push(somePostSec_25p_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (somePostSec_25p_female_rate_CO.calculable && somePostSec_25p_male_rate_CO.calculable) {

                completionObject.calculation_queue.push(...somePostSec_25p_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...somePostSec_25p_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
        ...
        case "completed-postsecondary":
            if (donePostSec_ratio != "") {

                completionObject.calculation_queue.push(donePostSec_ratio);
                completionObject.calculable = true;
                return completionObject;

            } else if (donePostSec_female_rate_CO.calculable && donePostSec_male_rate_CO.calculable) {

                completionObject.calculation_queue.push(...donePostSec_female_rate_CO.calculation_queue);
                completionObject.calculation_queue.push(...donePostSec_male_rate_CO.calculation_queue);
                completionObject.calculation_queue.push("division");
